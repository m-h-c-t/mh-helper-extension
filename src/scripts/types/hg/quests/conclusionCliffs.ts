...
const lengthType = ['short', 'medium', 'long'] as const;
...
export const questConclusionCliffsSchema = z.object({
    story: z.discriminatedUnion('is_writing', [
        z.object({
            is_writing: z.literal(false),
        }),
        z.object({
            is_writing: z.literal(true),
            is_postscript: z.boolean(),
            current_chapter: z.object({
                genre_type: genreTypeSchema,
                length_type: lengthTypeSchema,
            }),
            story_content: z.array(
                z.object({
                    genre_type: genreTypeSchema,
                }),
            ),
